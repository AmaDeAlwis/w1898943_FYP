...
# --- Patient ID and Previous Predictions ---
...
# --- Prediction Logic ---
if predict:
    if "" in [age, lymph_nodes, menopausal_status, tumor_stage, her2, er, pr, chemo, surgery, radio, hormone]:
        ...
        with st.container():
            ...
            st.write(f"**5-Year Survival Probability:** {surv_5yr:.2f} ({int(surv_5yr * 100)}%)")
            st.write(f"**10-Year Survival Probability:** {surv_10yr:.2f} ({int(surv_10yr * 100)}%)")
        ...
        with col1:
            ...
            for bar, val in zip(bars, [surv_5yr, surv_10yr]):
        ...
        c.drawString(100, 730, f"5-Year Survival: {surv_5yr:.2f} ({int(surv_5yr * 100)}%)")
        c.drawString(100, 710, f"10-Year Survival: {surv_10yr:.2f} ({int(surv_10yr * 100)}%)")
        c.save()
        pdf.seek(0)
